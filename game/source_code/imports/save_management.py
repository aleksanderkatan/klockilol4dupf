...
from source_code.imports.save_state import save_state
...
def _does_save_exist(index):
    return os.path.exists(f"user_data/save_slot_{index}")


def create_new_save(index, new_save_data):
    os.mkdir(f"user_data/save_slot_{index}")
...
def delete_save(index):
    ...
    os.rmdir(f"user_data/save_slot_{index}")
